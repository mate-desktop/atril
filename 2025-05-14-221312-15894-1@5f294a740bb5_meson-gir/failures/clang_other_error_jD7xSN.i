...
static int vf_load_font (DviParams *, DviFont *);
static void vf_free_macros (DviFont *);
...
static int vf_load_font(DviParams *params, DviFont *font)
{
 ...
 if(((unsigned)getc(p)) != 247 || ((unsigned)getc(p)) != 202)
  goto badvf;
 mlen = ((unsigned)getc(p));
 fseek(p, (long)mlen, 1);
 checksum = fugetn((p), 4);
 ...
 while(op >= 243 && op <= 246) {
  ...
  id = fugetn(p, op - 243 + 1);
  checksum = fugetn((p), 4);
  scale = fugetn((p), 4);
  design = fugetn((p), 4);


  scale = (((((z) >> 24) & 0xff) == 255) ? ((((((((z)) & 0xff) * ((scale))) >> 8) + (((((z)) >> 8) & 0xff) * ((scale)))) >> 8) + ((((z)) >> 16) & 0xff) * ((scale))) / (beta) - (alpha) : ((((((((z)) & 0xff) * ((scale))) >> 8) + (((((z)) >> 8) & 0xff) * ((scale)))) >> 8) + ((((z)) >> 16) & 0xff) * ((scale))) / (beta));
  design = (int)((params->tfm_conv * design) + 0.5);


  hdpi = (int)((params->mag * params->dpi * scale / design) + 0.5);
  vdpi = (int)((params->mag * params->vdpi * scale / design) + 0.5);
  n = ((unsigned)getc(p)) + ((unsigned)getc(p));
  name = mdvi_malloc(n + 1);
  fread(name, 1, n, p);
  name[n] = 0;
  __debug ((1 << 1), "(vf) %s: defined font `%s' at %.1fpt (%dx%d dpi)\n", font->fontname, name, (double)scale / (params->tfm_conv * 0x100000), hdpi, vdpi);
  ...
  if(ref == ((void*)0)) {
   mdvi_error(((char *) g_dgettext ("atril", "(vf) %s: could not load font `%s'\n")),
       font->fontname, name);
   goto error;
  }
  mdvi_free(name);
 ...
 while(op <= 242) {
  ...
  if (cc < 0 || cc > 65536) {
   ...
   mdvi_error(((char *) g_dgettext ("atril", "(vf) %s: unexpected character %d\n")),
       font->fontname, cc);
   goto error;
  }
  if(loc < 0 || cc < loc)
  ...
  if(font->chars[cc].offset) {
   mdvi_error(((char *) g_dgettext ("atril", "(vf) %s: character %d redefined\n")),
       font->fontname, cc);
   goto error;
  }

  __debug ((1 << 7), "(vf) %s: defined character %d (macro length %d)\n", font->fontname, cc, pl);

  font->chars[cc].width = pl + 1;
  font->chars[cc].code = cc;
  font->chars[cc].tfmwidth = (((((z) >> 24) & 0xff) == 255) ? ((((((((z)) & 0xff) * ((tfm))) >> 8) + (((((z)) >> 8) & 0xff) * ((tfm)))) >> 8) + ((((z)) >> 16) & 0xff) * ((tfm))) / (beta) - (alpha) : ((((((((z)) & 0xff) * ((tfm))) >> 8) + (((((z)) >> 8) & 0xff) * ((tfm)))) >> 8) + ((((z)) >> 16) & 0xff) * ((tfm))) / (beta));
  font->chars[cc].offset = mlen;
  font->chars[cc].loaded = 1;
  if(mlen + pl + 1 > msize) {
   msize = mlen + pl + 256;
   macros = (Uchar *)mdvi_realloc((macros), (msize) * sizeof(Uchar));
  }
 ...
 if(op != 248) {
  mdvi_error(((char *) g_dgettext ("atril", "(vf) %s: no postamble\n")), font->fontname);
  goto error;
 }


 if(msize > mlen) {
  macros = (Uchar *)mdvi_realloc((macros), (mlen) * sizeof(Uchar));
  msize = mlen;
 }

 __debug ((1 << 1)|(1 << 7), "(vf) %s: macros use %d bytes\n", font->fontname, msize);
...
badvf:
 mdvi_error(((char *) g_dgettext ("atril", "%s: File corrupted, or not a VF file.\n")), font->fontname);
error:
 if(font->chars)
