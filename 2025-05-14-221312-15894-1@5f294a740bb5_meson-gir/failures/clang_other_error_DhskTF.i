 ...
 char *prefix, char *arg)
{
 ...
 char *ptr;
 char *filename;
 double value[(sizeof(keys) / sizeof(keys[0]))];
 Uchar present[(sizeof(keys) / sizeof(keys[0]))];
 Buffer buffer;
 char *name;
 int i;
 double originx;
 ...
 *ptr++ = 0;
 ...
 while(*ptr) {
   ...
   *p++ = 0;
   if(*p == '"') {
    ...
    if(*p != '"')
     mdvi_warning(
     ((char *) g_dgettext ("atril", "%s: malformed value for key `%s'\n")),
      filename, keyname);
   } else
    val = p;
  }
  ...
  for(i = 0; i < (sizeof(keys) / sizeof(keys[0])); i++)
   if((strcasecmp((keys[i].name), (keyname)) == 0))
    break;
  if(i == (sizeof(keys) / sizeof(keys[0]))) {
   mdvi_warning(((char *) g_dgettext ("atril", "%s: unknown key `%s' ignored\n")),
         filename, keyname);
   continue;
  }
  if(keys[i].has_arg && val == ((void*)0)) {
   mdvi_warning(((char *) g_dgettext ("atril", "%s: no argument for key `%s', using defaults\n")),
         filename, keyname);
   val = keys[i].value;
  } else if(!keys[i].has_arg && val) {
   mdvi_warning(((char *) g_dgettext ("atril", "%s: argument `%s' ignored for key `%s'\n")),
 ...
 if(present[7])
  originy = value[7];
 if(present[10])
  hscale = value[10] / 100.0;
 if(present[11])
  vscale = value[11] / 100.0;
 if(present[2] && present[0])
  hsize = value[2] - value[0];
 if(present[3] && present[1])
  vsize = value[3] - value[1];
 if(present[4] || present[5]) {
  if(present[4] && !present[5])
   hscale = vscale = value[4] / (10.0 * hsize);
  else if(present[5] && !present[4])
   hscale = vscale = value[5] / (10.0 * vsize);
  else {
   hscale = value[4] / (10.0 * hsize);
   vscale = value[5] / (10.0 * vsize);
  }
 }
...
void epsf_special(DviContext *dvi, char *prefix, char *arg)
{
 ...
 w = (int)((box.bw * xf) + 0.5);
 h = (int)((box.bh * vf) + 0.5);
 x = (int)((box.ox * xf) + 0.5) + dvi->pos.hh;
 y = (int)((box.oy * vf) + 0.5) + dvi->pos.vv - h + 1;
 ...
 psfile = kpse_find_file (file, kpse_pict_format, 1);
 if (psfile) {
  dvi->device.draw_ps (dvi, psfile, x, y, w, h);
 } else {
  dvi->device.draw_rule(dvi, x, y, w, h, 0);
