...
void mdvi_hash_init(DviHashTable *hash)
{
 ...
 hash->nbucks = 0;
 hash->nkeys = 0;
 hash->hash_func = ((void*)0);
 hash->hash_comp = ((void*)0);
 hash->hash_free = ((void*)0);
}
...
void mdvi_hash_create(DviHashTable *hash, int size)
{
 ...
 hash->nbucks = size;
 hash->buckets = (DviHashBucket * *)mdvi_calloc((size), sizeof(DviHashBucket *));
 for(i = 0; i < size; i++)
  hash->buckets[i] = ((void*)0);
...
int mdvi_hash_add(DviHashTable *hash, DviHashKey key, void *data, int rep)
{
 ...
 if(rep != 2) {
  ...
  if(buck != ((void*)0)) {
   ...
   if(rep == 1)
    return -1;
   if(hash->hash_free != ((void*)0))
    hash->hash_free(buck->key, buck->data);
  }
 }
 if(buck == ((void*)0)) {
  buck = (DviHashBucket *)mdvi_malloc(sizeof(DviHashBucket));
...
static DviHashBucket *hash_remove(DviHashTable *hash, DviHashKey key)
{
 ...
 for(last = ((void*)0), buck = hash->buckets[hval]; buck; buck = buck->next) {
  if(hash->hash_comp(buck->key, key) == 0)
   break;
  last = buck;
 }
