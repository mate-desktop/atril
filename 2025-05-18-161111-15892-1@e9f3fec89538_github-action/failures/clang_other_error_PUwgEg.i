...
{
 if(font->in)
  fseek(font->in, (long)0, 0);
 else if((font->in = kpse_fopen_trace(font->filename, "rb")) == ((void*)0)) {
  __debug ((1 << 2), "reopen(%s) -> Error\n", font->filename);
  return -1;
 }
...
static int load_font_file(DviParams *params, DviFont *font)
{
 ...
 do {
  status = font->search.info->load(params, font);
...
int font_free_unused(DviDevice *dev)
{
 ...
 for(font = (DviFont *)fontlist.head; font; font = next) {
  ...
  font_reset_font_glyphs(dev, font, (1 << 2));

  if(font->search.info->freedata)
   font->search.info->freedata(font);
 ...
 __debug ((1 << 1), "%d unused fonts removed\n", count);
 return count;
}
 ...
 Int32 scale)
{
 ...
 if(!font->links && !font->chars && load_font_file(params, font) < 0) {
  __debug ((1 << 1), "font_reference(%s) -> Error\n", name);
  return ((void*)0);
 }
 ref = (DviFontRef *)mdvi_malloc(sizeof(DviFontRef));
 ref->ref = font;
...
static int load_one_glyph(DviContext *dvi, DviFont *font, int code)
{
 ...
 status = font->search.info->getglyph(&dvi->params, font, code);
 if(status < 0)
  return -1;
 ...
 if(!font->search.info->scalable && font->hdpi != font->vdpi) {
  int hs, vs, d;


  d = (((font->hdpi) > (font->vdpi)) ? (font->hdpi) : (font->vdpi));
  hs = d / font->hdpi;
  vs = d / font->vdpi;
  if(ch->width && ch->height && (hs > 1 || vs > 1)) {
   ...
   font->search.info->shrink0(dvi, font, ch, &glyph);

   dvi->params.hshrink = h;
   dvi->params.vshrink = v;
   dvi->params.density = d;

   if(!((ch->glyph.data) == ((void *)1)))
...
again:
 ...
 if(ch->missing || ((ch->glyph.data) == ((void *)1))) {
  ...
  font->search.info->shrink1(dvi, font, ch, &ch->grey);
 } else if(!ch->shrunk.data)
  font->search.info->shrink0(dvi, font, ch, &ch->shrunk);
...
void font_reset_one_glyph(DviDevice *dev, DviFontChar *ch, int what)
{
 if(!((ch) && (ch)->offset))
  return;
 if(what & (1 << 0)) {
  if(((ch->shrunk.data) && (ch->shrunk.data) != ((void *)1)))
   bitmap_destroy((BITMAP *)ch->shrunk.data);
  ch->shrunk.data = ((void*)0);
 }
 if(what & (1 << 1)) {
  if(((ch->grey.data) && (ch->grey.data) != ((void *)1))) {
   if(dev->free_image)
 ...
 if(what & (1 << 2)) {
  if(((ch->glyph.data) && (ch->glyph.data) != ((void *)1)))
   bitmap_destroy((BITMAP *)ch->glyph.data);
...
void font_reset_font_glyphs(DviDevice *dev, DviFont *font, int what)
{
 ...
 if(font->search.info->getglyph == ((void*)0))
  return;
 __debug ((1 << 1), "resetting glyphs in font `%s'\n", font->fontname);
 for(ch = font->chars, i = font->loc; i <= font->hic; ch++, i++) {
  if(((ch) && (ch)->offset))
   font_reset_one_glyph(dev, ch, what);
 }
 if((what & (1 << 2)) && font->search.info->reset)
  font->search.info->reset(font);
}
...
void font_finish_definitions(DviContext *dvi)
{
 ...
 if(dvi->fonts == ((void*)0)) {
  mdvi_warning(((char *) g_dgettext ("atril", "%s: no fonts defined\n")), dvi->filename);
  return;
 }
 map = (DviFontRef * *)mdvi_calloc((dvi->nfonts), sizeof(DviFontRef *));
