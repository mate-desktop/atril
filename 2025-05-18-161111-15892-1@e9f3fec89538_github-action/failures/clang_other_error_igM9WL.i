...
# 23 "/usr/include/kpathsea/tex-file.h" 3 4
# 1 "/usr/include/kpathsea/c-proto.h" 1 3 4
# 24 "/usr/include/kpathsea/tex-file.h" 2 3 4
# 1 "/usr/lib64/llvm20/bin/../../../lib/clang/20/include/stdarg.h" 1 3 4
...
# 26 "/usr/include/kpathsea/tex-file.h" 2 3 4
...
char *_mdvi_fallback_font = "cmr10";



static ListHead font_classes[3];
...
char **mdvi_list_font_class(int klass)
{
 ...
 list = (char * *)mdvi_calloc((n + 1), sizeof(char *));
...
int mdvi_register_font_type(DviFontInfo *info, int klass)
{
 ...
 if(!initialized)
  init_font_classes();
 fc = (struct _DviFontClass *)mdvi_malloc(sizeof(struct _DviFontClass));
 fc->links = 0;
 fc->id = klass;
...
int mdvi_unregister_font_type(const char *name, int klass)
{
 ...
 if(klass >= 0 && klass < 3) {
  k = klass;
  for(fc = (DviFontClass *)(&font_classes[k])->head; fc; fc = (fc)->next) {
   if((strcmp((fc->info.name), (name)) == 0))
    break;
  }
 } else if(klass < 0) {
  for(k = 0; k < 3; k++) {
...
static char *lookup_font(DviFontClass *ptr, const char *name, Ushort *h, Ushort *v)
{
 ...
 if(ptr->info.lookup)
  ...
  filename = kpse_find_glyph(name, (((*h) > (*v)) ? (*h) : (*v)),
...
again:
 ...
 if(kid < 3 - 1 && !(strcmp((name), (_mdvi_fallback_font)) == 0)) {
 ...
 if(kid == 3 -1) {
  ...
  mdvi_warning("font `%s' not found, trying metric files instead\n",
        name);
  ptr = (DviFontClass *)font_classes[3 -1].head;
 }

metrics:
 while(ptr) {
  __debug ((1 << 1), "metric: trying `%s' at (%d,%d)dpi as `%s'\n", name, hdpi, vdpi, ptr->info.name);
 ...
 if(filename != ((void*)0)) {
  if((strcmp((name), (_mdvi_fallback_font)) == 0))
   search->id = 3;
  else
   search->id = 3 - 1;
  search->curr = ptr;
 ...
 int hdpi, int vdpi, Int32 scale)
{
 ...
 font = (DviFont *)mdvi_malloc(sizeof(DviFont));
 font->fontname = mdvi_strdup(name);
 do { (font->search).id = 0; (font->search).curr = ((void*)0); (font->search).hdpi = (hdpi); (font->search).vdpi = (vdpi); (font->search).wanted_name = (font->fontname); (font->search).actual_name = ((void*)0); } while(0);
 font->filename = mdvi_lookup_font(&font->search);
 if(font->filename == ((void*)0)) {
...
int mdvi_font_retry(DviParams *params, DviFont *font)
{
 ...
 do { if(!(font->search.curr != ((void*)0))) mdvi_crash("%s:%d: Assertion %s failed\n", "fontsrch.c", 356, "font->search.curr != NULL"); } while(0);
