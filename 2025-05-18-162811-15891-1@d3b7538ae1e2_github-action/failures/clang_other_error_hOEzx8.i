...
# 27 "gf.c" 2
# 60 "gf.c"
static int gf_load_font (DviParams *, DviFont *);
static int gf_font_get_glyph (DviParams *, DviFont *, int);
...
static int gf_read_bitmap(FILE *p, DviFontChar *ch)
{
 ...
 if(op == 67) {
  ...
  ((unsigned)getc(p));
  min_m = ((unsigned)getc(p));
  max_m = ((unsigned)getc(p));
  min_n = ((unsigned)getc(p));
  max_n = ((unsigned)getc(p));
  min_m = max_m - min_m;
  min_n = max_n - min_n;
 } else {
  mdvi_error(((char *) g_dgettext ("atril", "GF: invalid opcode %d in character %d\n")),
      op, ch->code);
  return -1;
 }
 ...
 line = map->data;
 bpl = map->stride;
 __debug ((1 << 8), "(gf) reading character %d\n", ch->code);
 while((op = ((unsigned)getc(p))) != 69) {
  Int32 n;
  ...
  if(op == 0) {
   ...
   if(paint_switch == 1)
    bitmap_paint_bits(line + (x / (4 << 3)),
     x % (4 << 3), par);
   paint_switch = !paint_switch;
   x += par;
  } else if(op >= 74 && op <= 238) {
   y++;
   line = (BmUnit *)((Uchar *)(line) + (bpl));
   x = op - 74;
   paint_switch = 1;
   __debug ((1 << 8), "(gf) new_row_%d\n", x);
  } else switch(op) {
   case 70:
    y++;
    line = (BmUnit *)((Uchar *)(line) + (bpl));
   ...
   case 72:
   case 73:
    par = fugetn(p, op - 71 + 1);
    y += par + 1;
    line = (BmUnit *)((Uchar *)(line) + ((par + 1) * bpl));
    x = 0;
    paint_switch = 0;
    __debug ((1 << 8), "(gf) skip_%d\n", op - 71);
    break;
   case 239:
   case 240:
   case 241:
   case 242: {
    ...
    s = read_string(p, op - 239 + 1, ((void*)0), 0);
   ...
   case 244:
    ...
    break;
   default:
    mdvi_error(((char *) g_dgettext ("atril", "(gf) Character %d: invalid opcode %d\n")),
        ch->code, op);
    goto error;
  }
  ...
  if(x > ch->width || y > ch->height)
   goto toobig;
  __debug ((1 << 8), "(gf) curr_loc @ (%d,%d)\n", x, y);
 }

 if(op != 69)
  goto error;
 __debug ((1 << 8), "(gf) end of character %d\n", ch->code);
 return 0;

toobig:
 mdvi_error(((char *) g_dgettext ("atril", "(gf) character %d has an incorrect bounding box\n")),
     ch->code);
error:
 bitmap_destroy(map);
...
static int gf_load_font(DviParams *unused, DviFont *font)
{
 ...
 if(word && font->checksum && font->checksum != word) {
  mdvi_warning(((char *) g_dgettext ("atril", "%s: bad checksum (expected %u, found %u)\n")),
 ...
 while((op = ((unsigned)getc(p))) != 249) {
  ...
  case 246:
   ...
   break;
  default:
   mdvi_error(((char *) g_dgettext ("atril", "%s: junk in postamble\n")), font->fontname);
   goto error;
  }
  ch->code = cc;
  ch->tfmwidth = fugetn((p), 4);
  ch->tfmwidth = (((((z) >> 24) & 0xff) == 255) ? ((((((((z)) & 0xff) * ((ch->tfmwidth))) >> 8) + (((((z)) >> 8) & 0xff) * ((ch->tfmwidth)))) >> 8) + ((((z)) >> 16) & 0xff) * ((ch->tfmwidth))) / (beta) - (alpha) : ((((((((z)) & 0xff) * ((ch->tfmwidth))) >> 8) + (((((z)) >> 8) & 0xff) * ((ch->tfmwidth)))) >> 8) + ((((z)) >> 16) & 0xff) * ((ch->tfmwidth))) / (beta));
  ch->offset = fugetn((p), 4);
  if(ch->offset == -1)
   ch->offset = 0;
...
badgf:
 mdvi_error(((char *) g_dgettext ("atril", "%s: File corrupted, or not a GF file\n")), font->fontname);
error:
 if(font->chars) {
