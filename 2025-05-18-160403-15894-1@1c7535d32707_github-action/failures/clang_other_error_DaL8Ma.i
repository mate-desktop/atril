...
void mdvi_set_color(DviContext *dvi, Ulong fg, Ulong bg)
{
 if(dvi->curr_fg != fg || dvi->curr_bg != bg) {
  __debug ((1 << 6), "setting color to (%lu,%lu)\n", fg, bg);
...
static ColorCache color_cache[256];
    ...
    int nlevels, Ulong fg, Ulong bg, double gamma, int density)
{
 ...
 for(cc = &color_cache[0]; cc < &color_cache[cc_entries]; cc++) {
  ...
  if(cc->fg == fg && cc->bg == bg && cc->density == density &&
     cc->nlevels == nlevels && fabs(cc->gamma - gamma) <= 0.005)
      break;
 }
 ...
 __debug ((1 << 6), "Adding color table to cache (fg=%lu, bg=%lu, n=%d)\n", fg, bg, nlevels);
 ...
 pixels = (Ulong *)mdvi_calloc((nlevels), sizeof(Ulong));
