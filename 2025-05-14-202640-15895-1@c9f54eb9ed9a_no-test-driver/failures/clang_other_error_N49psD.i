...
# 34 "/usr/include/kpathsea/c-unistd.h" 3 4
# 1 "/usr/include/kpathsea/c-std.h" 1 3 4
...
# 1 "/usr/lib64/llvm20/bin/../../../lib/clang/20/include/inttypes.h" 1 3 4
# 24 "/usr/lib64/llvm20/bin/../../../lib/clang/20/include/inttypes.h" 3 4
...
extern uintmax_t wcstoumax (const __gwchar_t *__restrict __nptr,
       __gwchar_t ** __restrict __endptr, int __base)
     __attribute__ ((__nothrow__ ));
# 25 "/usr/lib64/llvm20/bin/../../../lib/clang/20/include/inttypes.h" 2 3 4
...
typedef int (*DviCommand) (DviContext *, int);



static int set_char (DviContext *, int);
static int set_rule (DviContext *, int);
static int no_op (DviContext *, int);
static int push (DviContext *, int);
static int pop (DviContext *, int);
static int move_right (DviContext *, int);
static int move_down (DviContext *, int);
static int move_w (DviContext *, int);
static int move_x (DviContext *, int);
static int move_y (DviContext *, int);
static int move_z (DviContext *, int);
static int sel_font (DviContext *, int);
static int sel_fontn (DviContext *, int);
static int special (DviContext *, int);
static int def_font (DviContext *, int);
static int undefined (DviContext *, int);
static int unexpected (DviContext *, int);
...
static const DviCommand dvi_commands[] = {
 ...
 move_w, move_w, move_w, move_w, move_w,
 move_x, move_x, move_x, move_x, move_x,
 move_down, move_down, move_down, move_down,
 move_y, move_y, move_y, move_y, move_y,
 move_z, move_z, move_z, move_z, move_z,
 ...
 sel_font, sel_font, sel_font, sel_font,
 sel_font, sel_font, sel_font, sel_font,
 sel_fontn, sel_fontn, sel_fontn, sel_fontn,
 special, special, special, special,
 def_font, def_font, def_font, def_font,
 unexpected,
 unexpected,
 unexpected,
...
static int get_bytes(DviContext *dvi, size_t n)
{
 ...
 if(((dvi)->buffer.pos + (n) > (dvi)->buffer.length)) {
  size_t required;
  int newlen;

  if(dvi->buffer.frozen || dvi->in == ((void*)0) || feof(dvi->in)) {

   dviwarn(dvi, ((char *) g_dgettext ("atril", "unexpected EOF\n")));
   return -1;
  }

  if(dvi->buffer.data == ((void*)0)) {

   dvi->buffer.size = (((4096) > (n)) ? (4096) : (n));
  ...
  if(required > dvi->buffer.size - dvi->buffer.length) {
   ...
   dvi->buffer.data = (Uchar *)(char *)mdvi_realloc((dvi->buffer.data), (dvi->buffer.size) * sizeof(char));
  ...
  if(newlen == -1) {
   mdvi_error("%s: %s\n", dvi->filename, strerror((*__errno_location ())));
   return -1;
  }
...
{
 do { if(!(offset > 0)) mdvi_crash("%s:%d: Assertion %s failed\n", "dviread.c", 253, "offset > 0"); } while(0);

 if(((dvi)->buffer.pos + (offset) > (dvi)->buffer.length) && get_bytes(dvi, offset) == -1)
  return -1;
...
static long dsgetn(DviContext *dvi, size_t n)
{
 ...
 if(((dvi)->buffer.pos + (n) > (dvi)->buffer.length) && get_bytes(dvi, n) == -1)
  return -1;
...
{
 if(((dvi)->buffer.pos + (len) > (dvi)->buffer.length) && get_bytes(dvi, len) == -1)
  return -1;
 bcopy ((dvi->buffer.data + dvi->buffer.pos), (buffer), (len));
 dvi->buffer.pos += len;
 return 0;
}
...
static DviFontRef *define_font(DviContext *dvi, int op)
{
 ...
 arg = dugetn(dvi, op - 243 + 1);
 checksum = dugetn((dvi), 4);
 scale = dugetn((dvi), 4);
 dsize = dugetn((dvi), 4);
 hdpi = (int)((dvi->params.mag * dvi->params.dpi * scale / dsize) + 0.5);
 vdpi = (int)((dvi->params.mag * dvi->params.vdpi * scale / dsize) + 0.5);
 n = dugetn((dvi), 1) + dugetn((dvi), 1);
 name = mdvi_malloc(n + 1);
 dread(dvi, name, n);
 name[n] = 0;
 __debug ((1 << 1), "requesting font %d = `%s' at %.1fpt (%dx%d dpi)\n", arg, name, (double)scale / (dvi->params.tfm_conv * 0x100000), hdpi, vdpi);


 ref = font_reference(&dvi->params, arg, name, checksum, hdpi, vdpi, scale);
 if(ref == ((void*)0)) {
  mdvi_error(((char *) g_dgettext ("atril", "could not load font `%s'\n")), name);
  mdvi_free(name);
  return ((void*)0);
 }
 mdvi_free(name);
...
static char *opendvi(const char *name)
{
 ...
 if(len >= 4 && (((name+len-4) != ((void*)0)) && ((".dvi") != ((void*)0)) && (strcmp (name+len-4, ".dvi") == 0))) {
  __debug ((1 << 3)|(1 << 2), "opendvi: Trying `%s'\n", name);
  if(access(name, 4) == 0)
...
int mdvi_reload(DviContext *dvi, DviParams *np)
{
 ...
 if(dvi->in) {
  kpse_fclose_trace(dvi->in);
  dvi->in = ((void*)0);
 }

 pars = np ? np : &dvi->params;
 __debug ((1 << 3), "%s: reloading\n", dvi->filename);


 newdvi = mdvi_init_context(pars, dvi->pagesel, dvi->filename);
 if(newdvi == ((void*)0)) {
  mdvi_warning(((char *) g_dgettext ("atril", "could not reload `%s'\n")), dvi->filename);
  return -1;
 }
 ...
 __debug ((1 << 3), "%s: reload successful\n", dvi->filename);
...
int mdvi_configure(DviContext *dvi, DviParamCode option, ...)
{
 ...
 while(option != MDVI_PARAM_LAST) {
  switch(option) {
  case MDVI_SET_DPI:
   np.dpi = np.vdpi = __builtin_va_arg(ap, Uint);
   reset_all = 1;
   break;
  case MDVI_SET_XDPI:
   np.dpi = __builtin_va_arg(ap, Uint);
   reset_all = 1;
   break;
  case MDVI_SET_YDPI:
   np.vdpi = __builtin_va_arg(ap, Uint);
   break;
  case MDVI_SET_SHRINK:
   np.hshrink = np.vshrink = __builtin_va_arg(ap, Uint);
   reset_font = (1 << 1)|(1 << 0);
   break;
  case MDVI_SET_XSHRINK:
   np.hshrink = __builtin_va_arg(ap, Uint);
   reset_font = (1 << 1)|(1 << 0);
   break;
  case MDVI_SET_YSHRINK:
   np.vshrink = __builtin_va_arg(ap, Uint);
   reset_font = (1 << 1)|(1 << 0);
   break;
  case MDVI_SET_ORIENTATION:
   np.orientation = __builtin_va_arg(ap, DviOrientation);
   reset_font = (1 << 2);
   break;
  case MDVI_SET_GAMMA:
   np.gamma = __builtin_va_arg(ap, double);
   reset_font = (1 << 1);
   break;
  case MDVI_SET_DENSITY:
   np.density = __builtin_va_arg(ap, Uint);
   reset_font = (1 << 0);
   break;
  case MDVI_SET_MAGNIFICATION:
   np.mag = __builtin_va_arg(ap, double);
   reset_all = 1;
   break;
  case MDVI_SET_DRIFT:
   np.hdrift = np.vdrift = __builtin_va_arg(ap, int);
   break;
  case MDVI_SET_HDRIFT:
   np.hdrift = __builtin_va_arg(ap, int);
   break;
  case MDVI_SET_VDRIFT:
   np.vdrift = __builtin_va_arg(ap, int);
   break;
  case MDVI_SET_FOREGROUND:
   np.fg = __builtin_va_arg(ap, Ulong);
   reset_font = (1 << 1);
   break;
  case MDVI_SET_BACKGROUND:
   np.bg = __builtin_va_arg(ap, Ulong);
   reset_font = (1 << 1);
   break;
  default:
   break;
  }
  option = __builtin_va_arg(ap, DviParamCode);
 }
 __builtin_va_end(ap);
 ...
 dvi->params = np;
 if((reset_font & (1 << 2)) && dvi->device.refresh) {
  dvi->device.refresh(dvi, dvi->device.device_data);
  return 0;
 }
...
DviContext *mdvi_init_context(DviParams *par, DviPageSpec *spec, const char *file)
{
 ...
 p = kpse_fopen_trace(filename, "rb");
 if(p == ((void*)0)) {
  perror(file);
  mdvi_free(filename);
  return ((void*)0);
 }
 dvi = (DviContext *)mdvi_malloc(sizeof(DviContext));
 ...
 if(((unsigned)getc(p)) != 247)
  goto bad_dvi;
 if((arg = ((unsigned)getc(p))) != 2) {
  mdvi_error(((char *) g_dgettext ("atril", "%s: unsupported DVI format (version %u)\n")),
      file, arg);
  goto error;
 ...
 dvi->params.dpi = par->dpi ? par->dpi : 600;
 ...
 if(fseek(p, (long)-1, 2) == -1)
  goto error;
 for(n = 0; (op = ((unsigned)getc(p))) == 223; n++)
  if(fseek(p, (long)-2, 1) < 0)
   break;
 if(op != arg || n < 4)
  goto bad_dvi;
 ...
 fseek(p, (long)arg, 0);
 if(((unsigned)getc(p)) != 248)
  goto bad_dvi;
 offset = fugetn((p), 4);
 if(dvi->num != fugetn((p), 4) || dvi->den != fugetn((p), 4) ||
    dvi->dvimag != fugetn((p), 4))
  goto bad_dvi;
 dvi->dvi_page_h = fugetn((p), 4);
 dvi->dvi_page_w = fugetn((p), 4);
 dvi->stacksize = fugetn((p), 2);
 dvi->npages = fugetn((p), 2);
 ...
 dvi->pagemap = (PageNum *)mdvi_calloc((dvi->npages), sizeof(PageNum));
 ...
 while(offset != -1) {
  ...
  if(spec && mdvi_page_selected(spec, page, n) == 0) {
   __debug ((1 << 3), "Page %d (%ld.%ld.%ld.%ld.%ld.%ld.%ld.%ld.%ld.%ld) ignored by request\n", n, page[1], page[2], page[3], page[4], page[5], page[6], page[7], page[8], page[9], page[10]);


  } else {
   bcopy ((page), (&dvi->pagemap[pagecount]), (sizeof(PageNum)));
   pagecount--;
  }
  n--;
 }
 ...
 if(pagecount >= dvi->npages) {
  mdvi_error(((char *) g_dgettext ("atril", "no pages selected\n")));
  goto error;
 }
 if(pagecount) {
 ...
 dvi->curr_layer = 0;
 dvi->stack = (DviState *)mdvi_calloc((dvi->stacksize + 8), sizeof(DviState));
 ...
 dvi->device.device_data = ((void*)0);

 __debug ((1 << 3), "%s read successfully\n", filename);
 return dvi;

bad_dvi:
 mdvi_error(((char *) g_dgettext ("atril", "%s: File corrupted, or not a DVI file\n")), file);
error:
...
static int mdvi_run_macro(DviContext *dvi, Uchar *macro, size_t len)
{
 ...
 while((opcode = dugetn((dvi), 1)) != 140) {
  if(dvi_commands[opcode](dvi, opcode) < 0)
   break;
 }
 if(opcode != 140)
  dviwarn(dvi, ((char *) g_dgettext ("atril", "%s: vf macro had errors\n")),
   curr->ref->fontname);
 if(dvi->stacktop != oldtop)
  dviwarn(dvi, ((char *) g_dgettext ("atril", "%s: stack not empty after vf macro\n")),
   curr->ref->fontname);
...
again:
 if(dvi->in == ((void*)0)) {
  ...
  dvi->in = kpse_fopen_trace(dvi->filename, "rb");
  if(dvi->in == ((void*)0)) {
   mdvi_warning(((char *) g_dgettext ("atril", "%s: could not reopen file (%s)\n")),
         dvi->filename,
         strerror((*__errno_location ())));
   return -1;
  }
  __debug ((1 << 2), "reopen(%s) -> Ok\n", dvi->filename);
 }
 ...
 if(pageno < 0 || pageno > dvi->npages-1) {
  mdvi_error(((char *) g_dgettext ("atril", "%s: page %d out of range\n")),
      dvi->filename, pageno);
  return -1;
 }

 fseek(dvi->in, (long)dvi->pagemap[pageno][0], 0);
 if((op = ((unsigned)getc(dvi->in))) != 139) {
  mdvi_error(((char *) g_dgettext ("atril", "%s: bad offset at page %d\n")),
      dvi->filename, pageno+1);
  return -1;
 }


 fseek(dvi->in, (long)44, 1);
 ...
 if(dvi->params.hdrift < 0) {
  ppi = dvi->params.dpi / dvi->params.hshrink;
  if(ppi < 600)
 ...
 if(dvi->params.vdrift < 0) {
  ppi = dvi->params.vdpi / dvi->params.vshrink;
  if(ppi < 600)
 ...
 dvi->params.thinsp = (int)((0.025 * dvi->params.dpi / dvi->params.conv) + 0.5);
 dvi->params.vsmallsp = (int)((0.025 * dvi->params.vdpi / dvi->params.vconv) + 0.5);


 while((op = dugetn((dvi), 1)) != 140) {
 ...
 if(op != 140)
  return -1;
 if(dvi->stacktop)
  dviwarn(dvi, ((char *) g_dgettext ("atril", "stack not empty at end of page\n")));
 return 0;
}
...
inline static int move_vertical(DviContext *dvi, int amount)
{
 ...
 dvi->pos.v += amount;
 rvv = (int)((dvi)->params.vconv * (dvi->pos.v) + 0.5);
 ...
 if(amount > dvi->params.vsmallsp || amount <= -dvi->params.vsmallsp)
  ...
  newvv = dvi->pos.vv + (int)((dvi)->params.vconv * (amount) + 0.5);
...
inline static int move_horizontal(DviContext *dvi, int amount)
{
 ...
 dvi->pos.h += amount;
 rhh = (int)((dvi)->params.conv * (dvi->pos.h) + 0.5);
 if(!dvi->params.hdrift)
  ...
  newhh = dvi->pos.hh + (int)((dvi)->params.conv * (amount) + 0.5);
...
{
 DviGlyph *glyph = ((void*)0);
 int x, y, w, h;

 if(!((ch->shrunk.data) == ((void*)0)))
  glyph = &ch->shrunk;
 else if(!((ch->grey.data) == ((void*)0)))
  glyph = &ch->grey;
 else if(!((ch->glyph.data) == ((void*)0)))
  glyph = &ch->glyph;
 if(glyph == ((void*)0))
  return;
 x = glyph->x;
...
int set_char(DviContext *dvi, int opcode)
{
 ...
 if(opcode < 128)
  ...
  num = dugetn(dvi, opcode - 128 + 1);
 if(dvi->currfont == ((void*)0)) {
  dvierr(dvi, ((char *) g_dgettext ("atril", "no default font set yet\n")));
  return -1;
 }
 font = dvi->currfont->ref;
 ch = font_get_glyph(dvi, font, num);
 if(ch == ((void*)0) || ch->missing) {

  ch = (((num) < font->loc || (num) > font->hic || !(font)->chars) ? ((void*)0) : &font->chars[(num) - (font)->loc]);
  if(!((ch) && (ch)->offset)) {
   dviwarn(dvi,
   ((char *) g_dgettext ("atril", "requested character %d does not exist in `%s'\n")),
    num, font->fontname);
   return 0;
  }
  draw_box(dvi, ch);
 } else if(dvi->curr_layer <= dvi->params.layer) {
  if(((font)->search.info->getglyph == ((void*)0)))
 ...
 if(opcode >= 133 && opcode <= 136) {
  if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, "putchar", opcode - 133 + 1, "char %d (%s)\n", num, dvi->currfont->ref->fontname); } while(0);


 } else {
  h = dvi->pos.h + ch->tfmwidth;
  hh = dvi->pos.hh + (int)((dvi)->params.conv * (ch->tfmwidth) + 0.5);
  if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, "setchar", num, "(%d,%d) h:=%d%c%d=%d, hh:=%d (%s)\n", dvi->pos.hh, dvi->pos.vv, (dvi->pos.h), (ch->tfmwidth) > 0 ? '+' : '-', (ch->tfmwidth) > 0 ? (ch->tfmwidth) : -(ch->tfmwidth), (h), hh, font->fontname); } while(0);
...
int set_rule(DviContext *dvi, int opcode)
{
 ...
 a = dsgetn((dvi), 4);
 b = dsgetn((dvi), 4); w = (int)((dvi)->params.conv * (b) + 0.99999);
 if(a > 0 && b > 0) {
  h = (int)((dvi)->params.vconv * (a) + 0.99999);
  if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, opcode == 132 ? "setrule" : "putrule", -1, "width %d, height %d (%dx%d pixels)\n", b, a, w, h); } while(0);
  ...
  if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, opcode == 132 ? "setrule" : "putrule", -1, "(moving left only, by %d)\n", b); } while(0);
...
{
 if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, "noop", -1, ""); } while(0);
 return 0;
}
...
int push(DviContext *dvi, int opcode)
{
 if(dvi->stacktop == dvi->stacksize) {
  if(!dvi->depth)
   dviwarn(dvi, ((char *) g_dgettext ("atril", "enlarging stack\n")));
  dvi->stacksize += 8;
  dvi->stack = (DviState *)mdvi_realloc((dvi->stack), (dvi->stacksize) * sizeof(DviState));

 }
 bcopy ((&dvi->pos), (&dvi->stack[dvi->stacktop]), (sizeof(DviState)));
 if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, "push", -1, "level %d: (h=%d,v=%d,w=%d,x=%d,y=%d,z=%d,hh=%d,vv=%d)\n", dvi->stacktop, dvi->pos.h, dvi->pos.v, dvi->pos.w, dvi->pos.x, dvi->pos.y, dvi->pos.z, dvi->pos.hh, dvi->pos.vv); } while(0);
...
{
 if(dvi->stacktop == 0) {
  dvierr(dvi, ((char *) g_dgettext ("atril", "stack underflow\n")));
  return -1;
 }
 bcopy ((&dvi->stack[dvi->stacktop-1]), (&dvi->pos), (sizeof(DviState)));
 if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, "pop", -1, "level %d: (h=%d,v=%d,w=%d,x=%d,y=%d,z=%d,hh=%d,vv=%d)\n", dvi->stacktop, dvi->pos.h, dvi->pos.v, dvi->pos.w, dvi->pos.x, dvi->pos.y, dvi->pos.z, dvi->pos.hh, dvi->pos.vv); } while(0);
...
int move_right(DviContext *dvi, int opcode)
{
 ...
 arg = dsgetn(dvi, opcode - 143 + 1);
 h = dvi->pos.h;
 hh = move_horizontal(dvi, arg);
 if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, "right", opcode - 143 + 1, "%d h:=%d%c%d=%d, hh:=%d\n", arg, (h), (arg) > 0 ? '+' : '-', (arg) > 0 ? (arg) : -(arg), (dvi->pos.h), hh); } while(0);
...
int move_down(DviContext *dvi, int opcode)
{
 ...
 arg = dsgetn(dvi, opcode - 157 + 1);
 v = dvi->pos.v;
 vv = move_vertical(dvi, arg);
 if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, "down", opcode - 157 + 1, "%d v:=%d%c%d=%d, vv:=%d\n", arg, (v), (arg) > 0 ? '+' : '-', (arg) > 0 ? (arg) : -(arg), (dvi->pos.v), vv); } while(0);
...
int move_w(DviContext *dvi, int opcode)
{
 ...
 if(opcode != 147)
  dvi->pos.w = dsgetn(dvi, opcode - 147);
 h = dvi->pos.h;
 hh = move_horizontal(dvi, dvi->pos.w);
 if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, "w", opcode - 147, "%d h:=%d%c%d=%d, hh:=%d\n", dvi->pos.w, (h), (dvi->pos.w) > 0 ? '+' : '-', (dvi->pos.w) > 0 ? (dvi->pos.w) : -(dvi->pos.w), (dvi->pos.h), hh); } while(0);
...
int move_x(DviContext *dvi, int opcode)
{
 ...
 if(opcode != 152)
  dvi->pos.x = dsgetn(dvi, opcode - 152);
 h = dvi->pos.h;
 hh = move_horizontal(dvi, dvi->pos.x);
 if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, "x", opcode - 152, "%d h:=%d%c%d=%d, hh:=%d\n", dvi->pos.x, (h), (dvi->pos.x) > 0 ? '+' : '-', (dvi->pos.x) > 0 ? (dvi->pos.x) : -(dvi->pos.x), (dvi->pos.h), hh); } while(0);
...
int move_y(DviContext *dvi, int opcode)
{
 ...
 if(opcode != 161)
  dvi->pos.y = dsgetn(dvi, opcode - 161);
 v = dvi->pos.v;
 vv = move_vertical(dvi, dvi->pos.y);
 if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, "y", opcode - 161, "%d h:=%d%c%d=%d, hh:=%d\n", dvi->pos.y, (v), (dvi->pos.y) > 0 ? '+' : '-', (dvi->pos.y) > 0 ? (dvi->pos.y) : -(dvi->pos.y), (dvi->pos.v), vv); } while(0);
...
int move_z(DviContext *dvi, int opcode)
{
 ...
 if(opcode != 166)
  dvi->pos.z = dsgetn(dvi, opcode - 166);
 v = dvi->pos.v;
 vv = move_vertical(dvi, dvi->pos.z);
 if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, "z", opcode - 166, "%d h:=%d%c%d=%d, hh:=%d\n", dvi->pos.z, (v), (dvi->pos.z) > 0 ? '+' : '-', (dvi->pos.z) > 0 ? (dvi->pos.z) : -(dvi->pos.z), (dvi->pos.v), vv); } while(0);
...
int sel_font(DviContext *dvi, int opcode)
{
 ...
 if(ref == ((void*)0)) {
  dvierr(dvi, ((char *) g_dgettext ("atril", "font %d is not defined\n")),
   opcode - 171);
  return -1;
 }
 if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, "fntnum", opcode - 171, "current font is %s\n", ref->ref->fontname); } while(0);
...
int sel_fontn(DviContext *dvi, int opcode)
{
 ...
 if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, "fnt", opcode - 235 + 1, "current font is %s (id %d)\n", ref->ref->fontname, arg); } while(0);
...
int special(DviContext *dvi, int opcode)
{
 ...
 arg = dugetn(dvi, opcode - 239 + 1);
 if (arg <= 0) {
  dvierr(dvi, ((char *) g_dgettext ("atril", "malformed special length\n")));
  return -1;
 }
 s = mdvi_malloc(arg + 1);
 dread(dvi, s, arg);
 ...
 if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, "XXXX", opcode - 239 + 1, "[%s]", s); } while(0);
...
int def_font(DviContext *dvi, int opcode)
{
 ...
 dskip(dvi, 12);
 dskip(dvi, dugetn((dvi), 1) + dugetn((dvi), 1));
 if(ref == ((void*)0)) {
  dvierr(dvi, ((char *) g_dgettext ("atril", "font %d is not defined in postamble\n")), arg);
  return -1;
 }
 if(_mdvi_debug_mask & (1 << 0)) do { dviprint (dvi, "fntdef", opcode - 243 + 1, "%d -> %s (%d links)\n", ref->fontid, ref->ref->fontname, ref->ref->links); } while(0);
...
{
 dvierr(dvi, ((char *) g_dgettext ("atril", "unexpected opcode %d\n")), opcode);
 return -1;
}

int undefined(DviContext *dvi, int opcode)
{
 dvierr(dvi, ((char *) g_dgettext ("atril", "undefined opcode %d\n")), opcode);
 return -1;
}
