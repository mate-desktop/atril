...
# 31 "/usr/include/tiffio.h" 3 4
# 1 "/usr/include/tiff.h" 1 3 4
# 28 "/usr/include/tiff.h" 3 4
# 1 "/usr/include/tiffconf.h" 1 3 4



# 1 "/usr/include/bits/wordsize.h" 1 3 4
# 5 "/usr/include/tiffconf.h" 2 3 4




# 1 "/usr/include/tiffconf-64.h" 1 3 4
# 18 "/usr/include/tiffconf-64.h" 3 4
# 1 "/usr/lib64/llvm20/bin/../../../lib/clang/20/include/stddef.h" 1 3 4
...
# 19 "/usr/include/tiffconf-64.h" 2 3 4
...
# 20 "/usr/include/tiffconf-64.h" 2 3 4
...
# 21 "/usr/include/tiffconf-64.h" 2 3 4
# 10 "/usr/include/tiffconf.h" 2 3 4
# 29 "/usr/include/tiff.h" 2 3 4
# 79 "/usr/include/tiff.h" 3 4
...
# 32 "/usr/include/tiffio.h" 2 3 4
# 1 "/usr/include/tiffvers.h" 1 3 4
# 33 "/usr/include/tiffio.h" 2 3 4
...
# 103 "/usr/include/tiffio.h" 3 4
typedef void *thandle_t;
# 135 "/usr/include/tiffio.h" 3 4
...
# 1 "/usr/lib64/llvm20/bin/../../../lib/clang/20/include/stdarg.h" 1 3 4
# 269 "/usr/include/tiffio.h" 2 3 4
# 286 "/usr/include/tiffio.h" 3 4
...
TIFF2PSContext* tiff2ps_context_new(const gchar *filename) {
 ...
 ctx = (TIFF2PSContext *) (__extension__ ({ gsize __n = (gsize) (1); gsize __s = sizeof (TIFF2PSContext); gpointer __p; if (__s == 1) __p = g_malloc0 (__n); else if (__builtin_constant_p (__n) && (__s == 0 || __n <= (9223372036854775807L *2UL+1UL) / __s)) __p = g_malloc0 (__n * __s); else __p = g_malloc0_n (__n, __s); __p; }));
 ctx->filename = g_strdup_inline (filename);
...
checkImage(TIFF2PSContext *ctx, TIFF* tif)
{
 ...
 case 6:
  if ((ctx->compression == 7
       || ctx->compression == 6)
      && ctx->planarconfiguration == 1) {
 ...
 case 32844:
 case 32845:
  if (ctx->compression != 34676 &&
      ctx->compression != 34677) {
   TIFFError(ctx->filename,
      "Can not handle %s data with ctx->compression other than SGILog",
       (ctx->photometric == 32844) ?
    "LogL" : "LogLuv"
   );
   return (0);
  }
  ...
  ctx->photometric = (ctx->photometric == 32844) ?
      1 : 2;
  ctx->bitspersample = 8;
  break;
 case 8:
...
# 300 "tiff2ps.c"
__attribute__((__format__ (__printf__, 6, 7))) static void
  ...
  const char* startline, ...)
{
 ...
 __builtin_va_start(args, startline);
        ...
        double* pprw, double* pprh)
{
 ...
 if (ctx->res_unit == 0)
  TIFFGetFieldDefaulted(tif, 296, &ctx->res_unit);
    ...
    double lm, double bm, int cnt)
{
 ...
 if (ctx->maxPageHeight==0)
  ...
  splitheight = ctx->maxPageHeight * 72.0F;
 overlap = ctx->splitOverlap * 72.0F;
...
PS_Lvl2colorspace(TIFF2PSContext* ctx, TIFF* tif)
{
 ...
 if (ctx->photometric != 3) {
  if (ctx->photometric == 6) {
...
# 834 "tiff2ps.c"
 ...
 if (ctx->planarconfiguration == 2 && ctx->samplesperpixel > 1)
 ...
 case 4:
  ...
  if (ctx->compression == 2 ||
      ctx->compression == 32771) {
   fputs("\t /EncodedByteAlign true\n", ctx->fd);
   fputs("\t /EndOfBlock false\n", ctx->fd);
  }
  if (ctx->photometric == 1)
   fputs("\t /BlackIs1 true\n", ctx->fd);
...
PSpage(TIFF2PSContext* ctx, TIFF* tif, uint32 w, uint32 h)
{
 ...
 case 1:
 case 0:
  PhotoshopBanner(ctx, w, h, 1, 1, ((ctx)->useImagemask && ((ctx)->bitspersample == 1)) ? "imagemask" : "image");
  fprintf(ctx->fd, "/scanLine %ld string def\n",
...
{
 uint32 row;
 int breaklen = 36, cc, es = ctx->samplesperpixel - nc;
 ...
 for (row = 0; row < h; row++) {
  ...
  if (ctx->alpha) {
   ...
   for (; cc < ctx->tf_bytesperrow; cc += ctx->samplesperpixel) {
    ...
    adjust = 255 - cp[nc];
    switch (nc) {
    case 4: c = *cp++ + adjust; putc(hex[((c)>>4)&0xf],ctx->fd); putc(hex[(c)&0xf],ctx->fd);
    case 3: c = *cp++ + adjust; putc(hex[((c)>>4)&0xf],ctx->fd); putc(hex[(c)&0xf],ctx->fd);
    case 2: c = *cp++ + adjust; putc(hex[((c)>>4)&0xf],ctx->fd); putc(hex[(c)&0xf],ctx->fd);
    case 1: c = *cp++ + adjust; putc(hex[((c)>>4)&0xf],ctx->fd); putc(hex[(c)&0xf],ctx->fd);
    }
    cp += es;
   }
  } else {
   ...
   for (; cc < ctx->tf_bytesperrow; cc += ctx->samplesperpixel) {
    if (((breaklen) -= (nc)) <= 0) { putc('\n', ctx->fd); (breaklen) = 36 -(nc); };
    switch (nc) {
    case 4: c = *cp++; putc(hex[((c)>>4)&0xf],ctx->fd); putc(hex[(c)&0xf],ctx->fd);
    case 3: c = *cp++; putc(hex[((c)>>4)&0xf],ctx->fd); putc(hex[(c)&0xf],ctx->fd);
    case 2: c = *cp++; putc(hex[((c)>>4)&0xf],ctx->fd); putc(hex[(c)&0xf],ctx->fd);
    case 1: c = *cp++; putc(hex[((c)>>4)&0xf],ctx->fd); putc(hex[(c)&0xf],ctx->fd);
    }
    cp += es;
   }
  }
 ...
 for (row = 0; row < h; row++) {
  for (s = 0; s < maxs; s++) {
   ...
   for (cp = tf_buf, cc = 0; cc < ctx->tf_bytesperrow; cc++) {
    if (((breaklen) -= (1)) <= 0) { putc('\n', ctx->fd); (breaklen) = 36 -(1); };
    c = *cp++;
    putc(hex[((c)>>4)&0xf],ctx->fd); putc(hex[(c)&0xf],ctx->fd);
   }
  }
 }
 _TIFFfree((char *) tf_buf);
...
PSDataPalette(TIFF2PSContext* ctx, TIFF* tif, uint32 w, uint32 h)
{
 ...
 for (row = 0; row < h; row++) {
  ...
  for (cp = tf_buf, cc = 0; cc < ctx->tf_bytesperrow; cc++) {
   ...
   switch (ctx->bitspersample) {
   case 8:
    c = *cp++; putc(hex[((rmap[c])>>4)&0xf],ctx->fd); putc(hex[(rmap[c])&0xf],ctx->fd); putc(hex[((gmap[c])>>4)&0xf],ctx->fd); putc(hex[(gmap[c])&0xf],ctx->fd); putc(hex[((bmap[c])>>4)&0xf],ctx->fd); putc(hex[(bmap[c])&0xf],ctx->fd);
    break;
   case 4:
    c = *cp++; putc(hex[((rmap[c&0xf])>>4)&0xf],ctx->fd); putc(hex[(rmap[c&0xf])&0xf],ctx->fd); putc(hex[((gmap[c&0xf])>>4)&0xf],ctx->fd); putc(hex[(gmap[c&0xf])&0xf],ctx->fd); putc(hex[((bmap[c&0xf])>>4)&0xf],ctx->fd); putc(hex[(bmap[c&0xf])&0xf],ctx->fd);
    c >>= 4; putc(hex[((rmap[c])>>4)&0xf],ctx->fd); putc(hex[(rmap[c])&0xf],ctx->fd); putc(hex[((gmap[c])>>4)&0xf],ctx->fd); putc(hex[(gmap[c])&0xf],ctx->fd); putc(hex[((bmap[c])>>4)&0xf],ctx->fd); putc(hex[(bmap[c])&0xf],ctx->fd);
    break;
   case 2:
    c = *cp++; putc(hex[((rmap[c&0x3])>>4)&0xf],ctx->fd); putc(hex[(rmap[c&0x3])&0xf],ctx->fd); putc(hex[((gmap[c&0x3])>>4)&0xf],ctx->fd); putc(hex[(gmap[c&0x3])&0xf],ctx->fd); putc(hex[((bmap[c&0x3])>>4)&0xf],ctx->fd); putc(hex[(bmap[c&0x3])&0xf],ctx->fd);
    c >>= 2; putc(hex[((rmap[c&0x3])>>4)&0xf],ctx->fd); putc(hex[(rmap[c&0x3])&0xf],ctx->fd); putc(hex[((gmap[c&0x3])>>4)&0xf],ctx->fd); putc(hex[(gmap[c&0x3])&0xf],ctx->fd); putc(hex[((bmap[c&0x3])>>4)&0xf],ctx->fd); putc(hex[(bmap[c&0x3])&0xf],ctx->fd);
    c >>= 2; putc(hex[((rmap[c&0x3])>>4)&0xf],ctx->fd); putc(hex[(rmap[c&0x3])&0xf],ctx->fd); putc(hex[((gmap[c&0x3])>>4)&0xf],ctx->fd); putc(hex[(gmap[c&0x3])&0xf],ctx->fd); putc(hex[((bmap[c&0x3])>>4)&0xf],ctx->fd); putc(hex[(bmap[c&0x3])&0xf],ctx->fd);
    c >>= 2; putc(hex[((rmap[c])>>4)&0xf],ctx->fd); putc(hex[(rmap[c])&0xf],ctx->fd); putc(hex[((gmap[c])>>4)&0xf],ctx->fd); putc(hex[(gmap[c])&0xf],ctx->fd); putc(hex[((bmap[c])>>4)&0xf],ctx->fd); putc(hex[(bmap[c])&0xf],ctx->fd);
    break;
   case 1:
    c = *cp++; putc(hex[((rmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(rmap[c&0x1])&0xf],ctx->fd); putc(hex[((gmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(gmap[c&0x1])&0xf],ctx->fd); putc(hex[((bmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(bmap[c&0x1])&0xf],ctx->fd);
    c >>= 1; putc(hex[((rmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(rmap[c&0x1])&0xf],ctx->fd); putc(hex[((gmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(gmap[c&0x1])&0xf],ctx->fd); putc(hex[((bmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(bmap[c&0x1])&0xf],ctx->fd);
    c >>= 1; putc(hex[((rmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(rmap[c&0x1])&0xf],ctx->fd); putc(hex[((gmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(gmap[c&0x1])&0xf],ctx->fd); putc(hex[((bmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(bmap[c&0x1])&0xf],ctx->fd);
    c >>= 1; putc(hex[((rmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(rmap[c&0x1])&0xf],ctx->fd); putc(hex[((gmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(gmap[c&0x1])&0xf],ctx->fd); putc(hex[((bmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(bmap[c&0x1])&0xf],ctx->fd);
    c >>= 1; putc(hex[((rmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(rmap[c&0x1])&0xf],ctx->fd); putc(hex[((gmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(gmap[c&0x1])&0xf],ctx->fd); putc(hex[((bmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(bmap[c&0x1])&0xf],ctx->fd);
    c >>= 1; putc(hex[((rmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(rmap[c&0x1])&0xf],ctx->fd); putc(hex[((gmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(gmap[c&0x1])&0xf],ctx->fd); putc(hex[((bmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(bmap[c&0x1])&0xf],ctx->fd);
    c >>= 1; putc(hex[((rmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(rmap[c&0x1])&0xf],ctx->fd); putc(hex[((gmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(gmap[c&0x1])&0xf],ctx->fd); putc(hex[((bmap[c&0x1])>>4)&0xf],ctx->fd); putc(hex[(bmap[c&0x1])&0xf],ctx->fd);
    c >>= 1; putc(hex[((rmap[c])>>4)&0xf],ctx->fd); putc(hex[(rmap[c])&0xf],ctx->fd); putc(hex[((gmap[c])>>4)&0xf],ctx->fd); putc(hex[(gmap[c])&0xf],ctx->fd); putc(hex[((bmap[c])>>4)&0xf],ctx->fd); putc(hex[(bmap[c])&0xf],ctx->fd);
    break;
   }
  }
 }
  ...
  if (ctx->ascii85) {
   ...
   if (ctx->alpha) {
    ...
    while (cc-- > 0) {
     ...
     adjust = 255 - cp[1];
     c = *cp++ + adjust; putc(hex[((c)>>4)&0xf],ctx->fd); putc(hex[(c)&0xf],ctx->fd);
     cp++, cc--;
    }
   } else {
    while (cc-- > 0) {
