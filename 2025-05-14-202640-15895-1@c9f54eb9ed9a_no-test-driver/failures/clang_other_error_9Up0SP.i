...
static const guint n_zoom_levels = (sizeof (zoom_levels) / sizeof ((zoom_levels)[0]));
...
ephy_zoom_get_changed_zoom_level (float level, gint steps)
{
 ...
 return zoom_levels[(((index + steps) > (n_zoom_levels - 1)) ? (n_zoom_levels - 1) : (((index + steps) < (3)) ? (3) : (index + steps)))].level;
}
