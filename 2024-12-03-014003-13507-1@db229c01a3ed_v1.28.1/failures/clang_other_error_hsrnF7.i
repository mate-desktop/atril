...
char *dgets(Dstring *dstr, FILE *in)
{
 ...
 if(feof(in))
  return ((void*)0);
 while(fgets(buffer, 256, in) != ((void*)0)) {
...
const char *file_extension(const char *filename)
{
 ...
 return (ptr ? ptr + 1 : ((void*)0));
}

int file_readable(const char *filename)
{
 int status = (access(filename, 4) == 0);

 __debug ((1 << 2), "file_redable(%s) -> %s\n", filename, status ? "Yes" : "No");
...
{
 int status = (access(filename, 0) == 0);

 __debug ((1 << 2), "file_exists(%s) -> %s\n", filename, status ? "Yes" : "No");
