...
{
 if(logfile != ((void*)0) && _mdvi_log_level >= level) {
  if(head != ((void*)0))
...
int mdvi_set_logfile(const char *filename)
{
 ...
 if(filename && (f = kpse_fopen_trace(filename, "w")) == ((void*)0))
  return -1;
 if(logfile != ((void*)0) && !isatty(fileno(logfile))) {
  kpse_fclose_trace(logfile);
  logfile = ((void*)0);
 }
 if(filename)
  logfile = f;
...
void mdvi_crash(const char *format, ...)
{
 ...
 fprintf(stderr, "%s: %s: ",
  program_name,
  ((char *) g_dgettext ("atril", messages[(int)time(((void*)0)) % (sizeof(messages) / sizeof(char *))])));
 vfprintf(stderr, format, ap);
 ...
 vputlog(2, ((char *) g_dgettext ("atril", "Crashing")), format, ap);
 __builtin_va_end(ap);
 abort();
}
...
void mdvi_error(const char *format, ...)
{
 ...
 __builtin_va_start(ap, format);
 fprintf(stderr, ((char *) g_dgettext ("atril", "%s: Error: ")), program_name);
 vfprintf(stderr, format, ap);
 ...
 vputlog(2, ((char *) g_dgettext ("atril", "Error")), format, ap);
 __builtin_va_end(ap);
}
...
void mdvi_warning(const char *format, ...)
{
 ...
 __builtin_va_start(ap, format);
 fprintf(stderr, ((char *) g_dgettext ("atril", "%s: Warning: ")), program_name);
 vfprintf(stderr, format, ap);
 ...
 vputlog(1, ((char *) g_dgettext ("atril", "Warning")), format, ap);
 __builtin_va_end(ap);
}
...
void mdvi_fatal(const char *format, ...)
{
 ...
 __builtin_va_start(ap, format);
 fprintf(stderr, ((char *) g_dgettext ("atril", "%s: Fatal: ")), program_name);
 vfprintf(stderr, format, ap);
 ...
 vputlog(2, ((char *) g_dgettext ("atril", "Fatal")), format, ap);
 __builtin_va_end(ap);
...
void *mdvi_malloc(size_t nelems)
{
 ...
 if(ptr == ((void*)0))
  mdvi_fatal(((char *) g_dgettext ("atril", "out of memory allocating %u bytes\n")),
      (unsigned)nelems);
 return ptr;
}
...
void *mdvi_realloc(void *data, size_t newsize)
{
 ...
 if(newsize == 0)
  mdvi_crash(((char *) g_dgettext ("atril", "attempted to reallocate with zero size\n")));
 ptr = realloc(data, newsize);
 if(ptr == ((void*)0))
  mdvi_fatal(((char *) g_dgettext ("atril", "failed to reallocate %u bytes\n")), (unsigned)newsize);
 return ptr;
}
...
void *mdvi_calloc(size_t nmemb, size_t size)
{
 ...
 if(nmemb == 0)
  mdvi_crash(((char *) g_dgettext ("atril", "attempted to callocate 0 members\n")));
 if(size == 0)
  mdvi_crash(((char *) g_dgettext ("atril", "attempted to callocate %u members with size 0\n")),
   (unsigned)nmemb);
 ptr = calloc(nmemb, size);
 if(ptr == 0)
  mdvi_fatal(((char *) g_dgettext ("atril", "failed to allocate %ux%u bytes\n")),
...
{
 if(ptr == ((void*)0))
  mdvi_crash(((char *) g_dgettext ("atril", "attempted to free NULL pointer\n")));
 free(ptr);
}
...
char *mdvi_build_path_from_cwd (const char *path)
{
 ...
 while (1) {
  ...
  if ((ptr = getcwd (buf, buf_size)) == ((void*)0) && (*__errno_location ()) == 34) {
   buf_size *= 2;
  } else {
   buf = ptr;
   break;
...
double unit2pix_factor(const char *spec)
{
 ...
 switch((int)(q - units)) {
        ...
        case 2: factor = 1.0 / 2.54; break;
        case 4: factor = 1.0 / 25.4; break;
        case 6: factor = 1.0 / 0.0254; break;
        case 8: factor = 1.0 / 72.27; break;
        case 10: factor = 12.0 / 72.27; break;
        case 12: factor = (1238.0 / 1157.0) / 72.27; break;
        case 14: factor = 12 * (1238.0 / 1157.0) / 72.27; break;
        case 16: factor = 1.0 / (72.27 * 65536); break;
        case 18: factor = 1.0 / 72.0; break;
         case 20: factor = 12.0; break;
         case 22: factor = 36.0; break;
        case 24: factor = 1.0 / 72000.0; break;
  default: factor = 1.0;
 }
 return factor * val;
}
...
Ulong get_mtime(int fd)
{
 ...
 if(fstat(fd, &st) == 0)
  return (Ulong)st.st_mtim.tv_sec;
 return 0;
}
...
int dstring_append(Dstring *dstr, const char *string, int len)
{
 ...
 if(len) {
  ...
  bcopy ((string), (dstr->data + dstr->length), (len));
  dstr->length += len;
  dstr->data[dstr->length] = 0;
 } else if(dstr->size == 0) {
  do { if(!(dstr->data == ((void*)0))) mdvi_crash("%s:%d: Assertion %s failed\n", "util.c", 487, "dstr->data == NULL"); } while(0);
  dstr->size = 8;
...
{
 do { if(!(pos >= 0)) mdvi_crash("%s:%d: Assertion %s failed\n", "util.c", 498, "pos >= 0"); } while(0);
 if(len < 0)
  len = strlen(string);
 if(len) {
  ...
  bcopy ((string), (dstr->data + pos), (len));
 }
 return dstr->length;
}
