...
# 40 "pk.c" 2
# 54 "pk.c"
static int pk_load_font (DviParams *, DviFont *);
static int pk_font_get_glyph (DviParams *, DviFont *, int);
...
static char *pk_lookup (const char *, Ushort *, Ushort *);
static char *pk_lookupn (const char *, Ushort *, Ushort *);
...
static char *pk_lookup(const char *name, Ushort *hdpi, Ushort *vdpi)
{
 ...
 filename = kpse_find_glyph(name, (((*hdpi) > (*vdpi)) ? (*hdpi) : (*vdpi)),
  kpse_pk_format, &type);
...
static BITMAP *get_bitmap(FILE *p, int w, int h, int flags)
{
 ...
 flags = 0;
 bitpos = -1;
 if((bm = bitmap_alloc(w, h)) == ((void*)0))
  return ((void*)0);
 __debug ((1 << 8), "get_bitmap(%d,%d,%d): reading raw bitmap\n", w, h, flags);
...
static BITMAP *get_packed(FILE *p, int w, int h, int flags)
{
 ...
 if((bm = bitmap_alloc(w, h)) == ((void*)0))
  return ((void*)0);
 __debug ((1 << 8), "get_packed(%d,%d,%d): reading packed glyph\n", w, h, flags);
 ...
 if(row != h || inrow != w) {
  mdvi_error(((char *) g_dgettext ("atril", "Bad PK file: More bits than required\n")));
  bitmap_destroy(bm);
  return ((void*)0);
 }
 return bm;
...
static int pk_load_font(DviParams *unused, DviFont *font)
{
 ...
 font->chars = (DviFontChar *)mdvi_calloc((256), sizeof(DviFontChar));
 p = font->in;
 ...
 loc = ((unsigned)getc(p)); hic = ((unsigned)getc(p));
 if(loc != 247 || hic != 89)
  goto badpk;
 i = ((unsigned)getc(p));

 for(loc = 0; loc < i; loc++)
  s[loc] = ((unsigned)getc(p));
 s[loc] = 0;
 __debug ((1 << 1), "(pk) %s: %s\n", font->fontname, s);
 ...
 font->design = fugetn((p), 4);

 checksum = fugetn((p), 4);
 if(checksum && font->checksum && font->checksum != checksum) {
  mdvi_warning(((char *) g_dgettext ("atril", "%s: checksum mismatch (expected %u, got %u)\n")),
 ...
 do { alpha = 16; z = (font->scale); while(z > 040000000L) { z >>= 1; alpha <<= 1; } beta = 256 / alpha; alpha *= z; } while(0);

 while((flag_byte = ((unsigned)getc(p))) != 245) {
  if(feof(p))
   break;
  if(flag_byte >= 240) {
   switch(flag_byte) {
   ...
   case 246:
    break;
   case 247:
    mdvi_error(((char *) g_dgettext ("atril", "%s: unexpected preamble\n")), font->fontname);
    goto error;
   }
  } else {
   int pl;
   ...
   case 5:
   case 6:
    pl = (flag_byte % 4) * 65536 + fugetn((p), 2);
    cc = ((unsigned)getc(p));
    offset = ftell(p) + pl;
    tfm = fugetn((p), 3);
    fsgetn((p), 2);
    ...
    break;
   default:
    pl = (flag_byte % 4) * 256 + ((unsigned)getc(p));
    cc = ((unsigned)getc(p));
    offset = ftell(p) + pl;
    tfm = fugetn((p), 3);
    fsgetn((p), 1);
   ...
   if (cc < 0 || cc > 65536) {
    mdvi_error (((char *) g_dgettext ("atril", "%s: unexpected charcode (%d)\n")),
         font->fontname,cc);
    goto error;
   }
   if(cc < loc)
   ...
   if(cc > hic)
    hic = cc;
   if(cc > maxch) {
    font->chars = (DviFontChar *)mdvi_realloc((font->chars), (cc + 16) * sizeof(DviFontChar));
   ...
   font->chars[cc].glyph.data = ((void*)0);
   font->chars[cc].x = x;
   font->chars[cc].y = y;
   ...
   font->chars[cc].grey.data = ((void*)0);
   font->chars[cc].shrunk.data = ((void*)0);
   font->chars[cc].tfmwidth = (((((tfm) >> 24) & 0xff) == 255) ? ((((((((tfm)) & 0xff) * ((z))) >> 8) + (((((tfm)) >> 8) & 0xff) * ((z)))) >> 8) + ((((tfm)) >> 16) & 0xff) * ((z))) / (beta) - (alpha) : ((((((((tfm)) & 0xff) * ((z))) >> 8) + (((((tfm)) >> 8) & 0xff) * ((z)))) >> 8) + ((((tfm)) >> 16) & 0xff) * ((z))) / (beta));
   font->chars[cc].loaded = 0;
   fseek(p, (long)offset, 0);
  }
 }
 if(flag_byte != 245) {
  mdvi_error(((char *) g_dgettext ("atril", "%s: unexpected end of file (no postamble)\n")),
      font->fontname);
  goto error;
 }
 while((flag_byte = ((unsigned)getc(p))) != (-1)) {
  if(flag_byte != 246) {
   mdvi_error(((char *) g_dgettext ("atril", "invalid PK file! (junk in postamble)\n")));
   goto error;
  }
 }
 ...
 if(loc > 0 || hic < maxch-1) {
  ...
  font->chars = (DviFontChar *)mdvi_realloc((font->chars), (hic - loc + 1) * sizeof(DviFontChar));
...
badpk:
 mdvi_error(((char *) g_dgettext ("atril", "%s: File corrupted, or not a PK file\n")), font->fontname);
error:
...
static int pk_font_get_glyph(DviParams *params, DviFont *font, int code)
{
 ...
 if((ch = (((code) < font->loc || (code) > font->hic || !(font)->chars) ? ((void*)0) : &font->chars[(code) - (font)->loc])) == ((void*)0))
  return -1;

 if(ch->offset == 0)
  return -1;
 __debug ((1 << 7), "(pk) loading glyph for character %d (%dx%d) in font `%s'\n", code, ch->width, ch->height, font->fontname);

 if(font->in == ((void*)0) && font_reopen(font) < 0)
  return -1;
 if(!ch->width || !ch->height) {
  ...
  ch->glyph.data = ((void*)0);
  return 0;
 }
 if(fseek(font->in, ch->offset, 0) == -1)
  return -1;
