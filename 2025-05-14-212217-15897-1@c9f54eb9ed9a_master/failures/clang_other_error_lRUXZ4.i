...
DviRange *mdvi_parse_range(const char *format, DviRange *limit, int *nitems, char **endptr)
{
  ...
  *cp = 0;
  ...
  if(*text) {
   upper_given = 1;
   t = strtol(text, ((void*)0), 0);
  }
  if(p == ((void*)0))
   goto finish;
  text = p;
  if(*text)
   s = strtol(text, ((void*)0), 0);
finish:
  if(lower_given && upper_given)
   this_type = MDVI_RANGE_BOUNDED;
  else if(lower_given) {
   if(!((type) == MDVI_RANGE_BOUNDED || (type) == MDVI_RANGE_UPPER))
    this_type = MDVI_RANGE_LOWER;
   else
    this_type = MDVI_RANGE_BOUNDED;
   t = upper;
  } else if(upper_given) {
   if(((one.type) == MDVI_RANGE_BOUNDED || (one.type) == MDVI_RANGE_UPPER)) {
    one.to++;
    this_type = MDVI_RANGE_BOUNDED;
   } else {
    one.to = lower;
    if(!((type) == MDVI_RANGE_BOUNDED || (type) == MDVI_RANGE_LOWER))
  ...
  bcopy ((&one), (&range[curr++]), (sizeof(DviRange)));
  *cp = ch;
  text = cp + 1;
 }
 if(done)
...
DviPageSpec *mdvi_parse_page_spec(const char *format)
{
 ...
 spec = (struct _DviPageSpec * *)mdvi_calloc((11), sizeof(struct _DviPageSpec *));
 for(i = 0; i < 11; i++)
  spec[i] = ((void*)0);


 if(*format != '*') {
  range = mdvi_parse_range(format, ((void*)0), &count, &ptr);
  if(ptr == format) {
   if(range) mdvi_free(range);
   mdvi_error(((char *) g_dgettext ("atril", "invalid page specification `%s'\n")), format);
   return ((void*)0);
  }
 } else
  range = ((void*)0);
 ...
 if(range) {
  spec[i] = (struct _DviPageSpec *)mdvi_malloc(sizeof(struct _DviPageSpec));
  spec[i]->ranges = range;
 ...
 if(*ptr != '.') {
  if(*ptr)
   mdvi_warning(((char *) g_dgettext ("atril", "garbage after DVI page specification ignored\n")));
  return spec;
 }
 ...
 for(i++; *ptr == '.' && i <= 10; i++) {
  ...
  if(*ptr == '*') {
   ...
   range = mdvi_parse_range(ptr, ((void*)0), &count, &end);
   if(end == ptr) {
    if(range) mdvi_free(range);
    range = ((void*)0);
   } else
 ...
 if(i > 10)
  mdvi_warning(((char *) g_dgettext ("atril", "more than 10 counters in page specification\n")));
 else if(*ptr)
  mdvi_warning(((char *) g_dgettext ("atril", "garbage after TeX page specification ignored\n")));
