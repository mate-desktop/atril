...
static ListHead tfmpool = {((void*)0), ((void*)0), 0};
...
int tfm_load_file(const char *filename, TFMInfo *info)
{
 ...
 size = 4 * (((st.st_size) + (4) - 1) / (4));
 if(size != st.st_size)
  mdvi_warning(((char *) g_dgettext ("atril", "Warning: TFM file `%s' has suspicious size\n")),
        filename);
 ...
 lf = (((ptr)) += (2), mugetn(((ptr))-(2), (2)));
 lh = (((ptr)) += (2), mugetn(((ptr))-(2), (2))); checksum += 6 + lh;
 bc = (((ptr)) += (2), mugetn(((ptr))-(2), (2)));
 ec = (((ptr)) += (2), mugetn(((ptr))-(2), (2))); checksum += ec - bc + 1;
 nw = (((ptr)) += (2), mugetn(((ptr))-(2), (2))); checksum += nw;
 nh = (((ptr)) += (2), mugetn(((ptr))-(2), (2))); checksum += nh;
 nd = (((ptr)) += (2), mugetn(((ptr))-(2), (2))); checksum += nd;
 checksum += (((ptr)) += (2), mugetn(((ptr))-(2), (2)));
 checksum += (((ptr)) += (2), mugetn(((ptr))-(2), (2)));
 checksum += (((ptr)) += (2), mugetn(((ptr))-(2), (2)));
 ne = (((ptr)) += (2), mugetn(((ptr))-(2), (2))); checksum += ne;
 checksum += (((ptr)) += (2), mugetn(((ptr))-(2), (2)));
 ...
 info->checksum = (((ptr)) += (4), mugetn(((ptr))-(4), (4)));

 info->design = (((ptr)) += (4), mugetn(((ptr))-(4), (4)));

 if(lh > 2) {

  i = n = (((ptr)) += (1), msgetn(((ptr))-(1), (1)));
  if(n < 0 || n > 39) {
   mdvi_warning(((char *) g_dgettext ("atril", "%s: font coding scheme truncated to 40 bytes\n")),
         filename);
   n = 39;
  }
  bcopy ((ptr), (info->coding), (n));
  info->coding[n] = 0;
  ptr += i;
 } else
 ...
 if(lh > 12) {
  n = (((ptr)) += (1), msgetn(((ptr))-(1), (1)));
  if(n > 0) {
   i = (((n) > (63)) ? (n) : (63));
   bcopy ((ptr), (info->family), (i));
   info->family[i] = 0;
  } else
 ...
 info->chars = (TFMChar *)mdvi_calloc((size), sizeof(TFMChar));
...
bad_tfm:
 mdvi_error(((char *) g_dgettext ("atril", "%s: File corrupted, or not a TFM file\n")), filename);
error:
 if(tfm) mdvi_free(tfm);
 if(in) kpse_fclose_trace(in);
 return -1;
}
...
static int ofm1_load_file(FILE *in, TFMInfo *info)
{
 ...
 info->checksum = fugetn((in), 4);

 info->design = fugetn((in), 4);
 ...
 for(tch = info->chars, i = 0; i < ncw; i++) {
  ...
  ch.advance = fugetn((in), 2);
  ch.height = ((unsigned)getc(in));
  ch.depth = ((unsigned)getc(in));

  fugetn((in), 4);

  nr = fugetn((in), 2);

  fseek(in, (long)npc * 2, 1);

  if(npc & 1) fugetn((in), 2);


  while(nr-- >= 0 && tch < end)
   bcopy ((&ch), (tch++), (sizeof(TFMChar)));
  if(tch == end)
   goto bad_tfm;
 }
 ...
 size = nw + nh + nd;
 tfm = (Int32 *)mdvi_calloc((size), sizeof(Int32));
...
static int ofm_load_file(const char *filename, TFMInfo *info)
{
 ...
 __debug ((1 << 1), "(mt) reading Level-0 OFM file `%s'\n", filename);
 nwords = 14;
 lf = fugetn((in), 4); checksum = nwords;
 lh = fugetn((in), 4); checksum += lh;
 bc = fugetn((in), 4);
 ec = fugetn((in), 4); checksum += 2 * (ec - bc + 1);
 nw = fugetn((in), 4); checksum += nw;
 nh = fugetn((in), 4); checksum += nh;
 nd = fugetn((in), 4); checksum += nd;
 checksum += fugetn((in), 4);
 checksum += 2*fugetn((in), 4);
 checksum += fugetn((in), 4);
 checksum += 2*fugetn((in), 4);
 checksum += fugetn((in), 4);
 ...
 fseek(in, 4L*(nwords + lh), 0);

 size = 2*(ec - bc + 1) + nw + nh + nd;
 tfm = (Int32 *)mdvi_calloc((size * sizeof(Int32)), sizeof(Int32));
 if(fread(tfm, 4, size, in) != size) {
  mdvi_free(tfm);
  goto bad_tfm;
 }
 ...
 for(i = bc; i <= ec; ptr += 4, i++) {
  ...
  ndx = (((ptr)) += (2), mugetn(((ptr))-(2), (2)));
  info->chars[i-bc].advance = widths[ndx];

  info->chars[i-bc].left = 0;
  info->chars[i-bc].right = widths[ndx];
  info->chars[i-bc].present = (ndx != 0);
  ndx = (((ptr)) += (1), mugetn(((ptr))-(1), (1)));
  info->chars[i-bc].height = heights[ndx];
  ndx = (((ptr)) += (1), mugetn(((ptr))-(1), (1)));
  info->chars[i-bc].depth = depths[ndx];
 }
...
char *lookup_font_metrics(const char *name, int *type)
{
 ...
 switch(*type) {
  ...
  case DviFontTFM:
   file = kpse_find_file (name, kpse_tfm_format, 1);
                        *type = DviFontTFM;
   break;
  case DviFontOFM: {
   file = kpse_find_file (name, kpse_ofm_format, 1);

   if(file != ((void*)0)) {
    const char *ext = file_extension(file);
    if(ext && (((ext) != ((void*)0)) && (("tfm") != ((void*)0)) && (strcmp (ext, "tfm") == 0)))
     *type = DviFontTFM;
   }
   break;
  }
...
TFMInfo *get_font_metrics(const char *short_name, int type, const char *filename)
{
 ...
 tfm = (TFMPool *)mdvi_malloc(sizeof(TFMPool));
 __debug ((1 << 1), "(mt) loading font metric data from `%s'\n", file, file);
 switch(type) {
 case DviFontTFM:
 ...
 if(tfmpool.count == 0)
  mdvi_hash_create(&tfmhash, 31);
 mdvi_hash_add(&tfmhash, ((DviHashKey)(tfm->short_name)),
  tfm, 2);
...
void free_font_metrics(TFMInfo *info)
{
 ...
 mdvi_hash_remove_ptr(&tfmhash, ((DviHashKey)(tfm->short_name)));

 __debug ((1 << 1), "(mt) removing unused TFM data for `%s'\n", tfm->short_name);
